...
const AppointmentPage = () => {
  ...
  // NEW FEATURE - Compute fully closed dates for calendar exclusion
  ...
  // Load available times for selected date (KEEPING MY VERSION - more comprehensive)
  useEffect(() => {
      ...
      try {
        ...
        const timeSlots: TimeSlot[] = await Promise.all(
          standardTimeSlots.map(async (time) => {
            ...
            // Check if this time slot conflicts with salon closures
            const closure = closures.find(c => c.date === dateStr);
            if (closure) {
              if (closure.startTime && closure.endTime) {
                // Check if time slot falls within closure period
    ...
    fetchAvailableTimes();
  }, [selectedDate, closures]); // Added closures dependency

  // NEW FEATURE - Fetch closures and set closure message for today or next upcoming closure
  useEffect(() => {
  ...
  // Handler for booking (KEEPING MY VERSION - navigates to payment)
  ...
  // Alert Modal Component (KEEPING MY VERSION - more comprehensive styling)
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-[#181818] rounded-xl border border-gray-700 p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Calendar - ENHANCED with closure exclusions */}
                <div>
                  ...
                  <div>
                    <DatePicker
                      ...
                      excludeDates={fullyClosedDates} // NEW FEATURE - Exclude fully closed dates
                      inline
                      className="w-full"
                ...
                {/* Time Slots - ENHANCED with my capacity system + closure handling */}
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {availableTimes.map((timeSlot) => (
                      <button
                        ...
                        title={
                          !timeSlot.available 
                            ? "This time slot is not available - staff capacity reached or salon closure" 
